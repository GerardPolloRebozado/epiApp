...
export default function Profile() {
	...
	useEffect(() => {
		...
		(async function () {
			...
			const formatLogTime: barDataItem[] = await current_week.map(
				(log: Array<any>) => {
					const value = log[1] / 3600;
					const label = new Date(log[0] * 1000)
						...
						<Accordion type={"multiple"} collapsable marginTop={"4"}>
							{marks.modules.map((module) => (
								<Accordion.Item
									...
									key={module.codemodule + module.codeinstance}
								>
									<Accordion.Trigger
										flexDirection="row"
