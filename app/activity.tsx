...
import type { ActivityExtendedType, AppointmentType, ProjectType } from '@/types';
    ...
    async function getActivity() {
      ...
      if (res.ok) {
        ...
        if (activityRes.is_projet) {
          ...
          if (projectBody.urls) {
            const tempRelActivitiesArray: ActivityExtendedType[] = [];
    ...
    <SafeAreaView style={styles.container}>
      ...
      <YStack width={'95vw'} gap={'$2'} justifyContent={'center'} alignItems={'center'}>
        {activity.description && (
          <Card radiused padded bordered backgrounded marginVertical={'$6'} borderWidth={'$1.5'}>
            <Card.Header>
          ...
          <ScrollView>
            <YStack style={styles.scrollView}>
              ...
              {activity.events.map((event) => {
                return (
                  <Card justifyContent={'center'} padded radiused elevate marginTop={'$2'} key={event.code} bordered>
                    <Card.Header>
                    ...
                    <Card.Footer>
                      <YStack>
                        <XStack width={'100%'} justifyContent={'space-between'}>
                          <Paragraph size={'$4'}>
                            {event.location && event.location.split('/').length > 2 ? (
                              <>
                  ...
                  return (
                    <YStack key={slot.id}>
                      <Accordion.Item value={slot.id.toString(10)}>
                        <Accordion.Trigger>
                          <Paragraph>
                            {slot.title.length > 0 ? slot.title : 'Appointment slot'} - {slot.room}
                          </Paragraph>
                        </Accordion.Trigger>
                        <Accordion.Content>
