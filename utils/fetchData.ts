...
export async function fetchImage(session: string): Promise<string> {
  const response = await fetchEpitech(
    `file/userprofil/profilview/${JWT.decode(session || 'error', null).login}.jpg`,
    session,
  );
...
export async function fetchActivity(
  ...
  return await fetchEpitech(
    `module/${year}/${module}/${city}/${activity}/${project ? 'project/' : ''}?format=json`,
    session,
  );
}
...
export async function registerActivity(
  ...
  return await fetchEpitech(
    `module/${year}/${module}/${city}/${activity}/${event}/${register ? 'register' : 'unregister'}?format=json`,
    session,
    'POST',
  );
}
...
export async function fetchAppointments({
  ...
  city,
  activity,
}: { session: string; year: string; module: string; city: string; activity: string }) {
...
export async function registerAppointment({
  ...
  activity,
  register,
}: { session: string; year: string; module: string; city: string; activity: string; register: boolean }) {
  return await fetchEpitech(
    `module/${year}/${module}/${city}/${activity}/rdv/${register ? 'register' : 'unregister'}?format=json`,
    session,
    'POST',
  );
}
